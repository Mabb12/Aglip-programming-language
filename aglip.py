def compile_aglip(code):
    ...
    counter = 0
    for cmd in code:
        if cmd == "A": py_code += "    " * in_loop + "memory[counter] = (memory[counter] + 1) % 256\n" # Add
        elif cmd == "G": # Go
            py_code += "    " * in_loop + "counter = (counter + 1) % 256\n"
            counter += 1 % 256
        elif cmd == "L": # Loop
            if in_loop == 0:
                py_code += "    " * in_loop + f"while memory[{counter}] != memory[{counter + 1}]:\n"
            in_loop = 1 - in_loop
        elif cmd == "I": py_code += "    " * in_loop + "memory[counter] = int(input()) % 256\n" # Input
